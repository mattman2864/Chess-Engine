    ...
    while running:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                elif not square_selected or gs.board[square_selected[0]][square_selected[1]] == '--':
                    moves = []
...
def draw_moves(screen, board, moves):
    for move in moves:
        pygame.draw.ellipse(screen, ATTACK, pygame.Rect(move.end_col*SQUARE_SIZE + (SQUARE_SIZE/3), move.end_row*SQUARE_SIZE + (SQUARE_SIZE/3), SQUARE_SIZE/3, SQUARE_SIZE/3))
